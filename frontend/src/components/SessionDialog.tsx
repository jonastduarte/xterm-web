...
const SessionDialog: React.FC<SessionDialogProps> = ({ onClose, onSave, initialData, mode = 'create' }) => {
  ...
  useEffect(() => {
    ...
    let defaultPort = 22;
    if (protocolLower === 'ftp') defaultPort = 21;
    if (protocolLower === 'telnet') defaultPort = 23;
    if (protocolLower === 'rdp') defaultPort = 3389;
    if (protocolLower === 'vnc') defaultPort = 5900;
    
    setFormData(prev => ({ 
      ...prev, 
      protocol: protocolLower,
      port: prev.port === 22 || prev.port === 21 || prev.port === 23 || prev.port === 3389 || prev.port === 5900 ? defaultPort : prev.port 
    }));
  }, [activeTab]);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      <div style={sectionStyle}>
        <h4 style={sectionTitleStyle}>
          <Terminal size={16} color="#005a9e" style={{ marginRight: '6px' }} />
          Basic SSH settings
        </h4>
        ...
        <div style={{ display: 'flex', gap: '12px', marginBottom: '12px' }}>
          <div style={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
            ...
            <input 
              type="text" 
              value={formData.name || ''} 
              placeholder={formData.host || 'e.g. Production Server'}
        ...
        <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-end' }}>
          <div style={{ display: 'flex', flexDirection: 'column', flex: 2 }}>
            ...
            <input 
              type="text" 
              value={formData.host} 
              placeholder="192.168.0.1 or hostname"
              onChange={e => setFormData({...formData, host: e.target.value})} 
              style={inputStyle}
            />
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
            <label style={labelStyle}>Username <span style={{ color: '#c0392b' }}>*</span></label>
            <input 
              type="text" 
      ...
      <div style={sectionStyle}>
        <h4 style={sectionTitleStyle}>
          <Lock size={16} color="#27ae60" style={{ marginRight: '6px' }} />
          Authentication
        </h4>
        ...
        <div style={{ display: 'flex', gap: '16px', marginBottom: '12px' }}>
          <label style={{ ...labelStyle, display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer' }}>
            <input 
              ...
              onChange={() => setFormData({...formData, auth_type: 'password'})} 
            />
            Password
          </label>
          <label style={{ ...labelStyle, display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer' }}>
            <input 
              ...
              onChange={() => setFormData({...formData, auth_type: 'key'})} 
            />
        ...
        {formData.auth_type === 'key' && (
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            ...
            <textarea 
              ...
              style={{ ...inputStyle, height: '80px', fontFamily: 'monospace', fontSize: '11px', resize: 'vertical' }}
              placeholder="-----BEGIN RSA PRIVATE KEY-----&#10;..."
            />
            <div style={{ display: 'flex', flexDirection: 'column', width: '250px', marginTop: '8px' }}>
              <label style={labelStyle}>Key Passphrase (if any)</label>
              <input 
                type="password" 
                value={formData.password} 
                onChange={e => setFormData({...formData, password: e.target.value})} 
      ...
      <div style={sectionStyle}>
        <label style={{ ...labelStyle, display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
          <input 
            type="checkbox" 
            checked={formData.use_sftp === 1}
            onChange={e => setFormData({...formData, use_sftp: e.target.checked ? 1 : 0})}
          />
          Enable SFTP browser (file panel on connect)
        </label>
      </div>
    </div>
  );
  ...
  const renderSFTPForm = () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      <div style={sectionStyle}>
        <h4 style={sectionTitleStyle}>
          <FileText size={16} color="#8e44ad" style={{ marginRight: '6px' }} />
          SFTP Connection Settings
        </h4>
        <p style={{ fontSize: '11px', color: '#888', margin: '0 0 12px 0' }}>
          Standalone SFTP file browser — connects without opening a terminal shell.
        </p>
        ...
        <div style={{ display: 'flex', gap: '12px', marginBottom: '12px' }}>
          <div style={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
            ...
            <input 
              type="text" 
              value={formData.name || ''} 
              placeholder={formData.host || 'e.g. File Server'}
              onChange={e => setFormData({...formData, name: e.target.value})} 
              style={inputStyle}
            />
          </div>
  ...
  const renderFTPForm = () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      <div style={sectionStyle}>
        <h4 style={sectionTitleStyle}>
          <Globe size={16} color="#f39c12" style={{ marginRight: '6px' }} />
          FTP Connection Settings
        </h4>
        <p style={{ fontSize: '11px', color: '#888', margin: '0 0 12px 0' }}>
          Standard FTP file transfer — browse, upload, and download files.
        </p>
        ...
        <div style={{ display: 'flex', gap: '12px', marginBottom: '12px' }}>
          <div style={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
            ...
            <input 
              type="text" 
              value={formData.name || ''} 
              placeholder={formData.host || 'e.g. FTP Server'}
              onChange={e => setFormData({...formData, name: e.target.value})} 
              style={inputStyle}
            />
          </div>
        ...
        <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-end' }}>
          <div style={{ display: 'flex', flexDirection: 'column', flex: 2 }}>
            <label style={labelStyle}>FTP Host <span style={{ color: '#c0392b' }}>*</span></label>
            <input 
              type="text" 
              value={formData.host}
      ...
      <div style={{ width: '820px', maxHeight: '90vh', backgroundColor: '#f0f0f0', borderRadius: '6px', boxShadow: '0 8px 30px rgba(0,0,0,0.3)', display: 'flex', flexDirection: 'column', border: '1px solid #999', fontFamily: "'Segoe UI', Tahoma, Geneva, Verdana, sans-serif", overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 16px', backgroundColor: '#fff', borderBottom: '1px solid #ccc' }}>
           <h3 style={{ margin: 0, fontSize: '15px', fontWeight: '600', color: '#333' }}>
             {mode === 'edit' ? 'Edit Session' : mode === 'clone' ? 'Clone Session' : 'New Session'}
           </h3>
        ...
        <div style={{ display: 'flex', backgroundColor: '#fff', padding: '8px 12px', gap: '8px', borderBottom: '1px solid #ccc', overflowX: 'auto' }}>
           {sessionTypes.map(type => (
             <div 
               key={type.id} 
               onClick={() => type.enabled && setActiveTab(type.id)}
               style={{ 
                 display: 'flex', flexDirection: 'column', alignItems: 'center', 
                 cursor: type.enabled ? 'pointer' : 'not-allowed', 
                 padding: '8px 10px', 
                 border: activeTab === type.id ? '2px solid #3498db' : '2px solid transparent', 
                 backgroundColor: activeTab === type.id ? '#e8f4f8' : 'transparent', 
                 borderRadius: '6px', 
                 minWidth: '56px',
           ...
           <button 
             onClick={() => {
               const saveData = { 
                 ...
                 protocol: activeTab.toLowerCase()
               };
...
const sectionStyle: React.CSSProperties = {
  backgroundColor: '#fff', border: '1px solid #ddd', padding: '16px', borderRadius: '6px'
};

const sectionTitleStyle: React.CSSProperties = {
  margin: '0 0 14px 0', fontSize: '13px', color: '#333', fontWeight: '600', display: 'flex', alignItems: 'center'
};

const labelStyle: React.CSSProperties = {
  fontSize: '12px', marginBottom: '4px', fontWeight: '500', color: '#555'
};

const inputStyle: React.CSSProperties = {
  padding: '7px 10px', border: '1px solid #ccc', borderRadius: '4px', 
  color: '#000', backgroundColor: '#fff', fontSize: '13px', outline: 'none',
  transition: 'border-color 0.15s'
};

const saveBtnStyle: React.CSSProperties = {
  padding: '8px 24px', backgroundColor: '#005a9e', border: 'none', 
  color: '#fff', borderRadius: '4px', cursor: 'pointer', fontSize: '13px', fontWeight: '500',
  transition: 'background 0.15s'
};

const cancelBtnStyle: React.CSSProperties = {
  padding: '8px 24px', backgroundColor: '#fff', border: '1px solid #ccc', 
  color: '#666', borderRadius: '4px', cursor: 'pointer', fontSize: '13px'
};
